...
export default function TransactionsPage() {
  ...
  const typeColor = {
    ...
    set: 'bg-indigo-100 text-indigo-700 border-indigo-200',
  }
    ...
    <div className="p-6 space-y-4 flex flex-col h-full">
      ...
      <div className="relative">
        ...
        <input value={filter} onChange={e => setFilter(e.target.value)} placeholder="Filter transactions..."
          className="w-full bg-white/80 backdrop-blur-xl border border-white/30 rounded-xl pl-9 pr-4 py-2.5 text-slate-800 text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm" />
      </div>
        ...
        {loading ? (
          ...
          <div className="overflow-auto flex-1">
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-gradient-to-r from-indigo-50 to-purple-50 backdrop-blur-sm">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  <tr key={tx.id} className="border-b border-slate-200/50 hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="px-5 py-3 text-right">
                      <span className={`font-bold ${tx.type === 'add' ? 'text-emerald-600' : tx.type === 'remove' ? 'text-red-600' : 'text-indigo-600'}`}>
